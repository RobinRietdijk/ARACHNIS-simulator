<template>
    <v-card>
        ...
        <v-card-text class="pb-0">
            <v-container>
                <v-row v-for="row in leg_rows" :key="row.name">
                    <v-col v-for="leg in row.legs" :key="leg.name">
                        <v-row><span class="pb-1"><b>{{ leg.name }}</b></span></v-row>
                        <v-row>
                            <v-text-field class="pa-1" label="alpha" v-model="leg.alpha" density="compact"
                                variant="outlined"
                                :rules="[rules.isNum, rules.notEmpty, rules.max(leg.alpha, 90), rules.min(leg.alpha, 0)]" />
                            <v-text-field class="pa-1" label="beta" v-model="leg.beta" density="compact" variant="outlined"
                                :rules="[rules.isNum, rules.notEmpty, rules.max(leg.beta, 180), rules.min(leg.beta, 0)]" />
                            <v-text-field class="pa-1" label="gamma" v-model="leg.gamma" density="compact"
                                variant="outlined"
